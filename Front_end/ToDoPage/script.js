...
async function saveToAPI(){
  await fetch('https://localhost:5001/ToDoCards/AddNewToDoCard', {
    method: 'POST',
    headers: {
...
async function saveEditAPI(){
  await fetch(`https://localhost:5001/ToDoCards/UpdateToDoCard/${toDoId}`, {
    method: 'PUT',
    headers: {
     'Content-type': 'application/json'
    },
...
function getToDo() {
  fetch('https://localhost:5001/ToDoCards/GetAllToDoCards')
  .then((res) => {
    if (res.ok) {
      return res.json();
    }
...
async function deleteToDo(toDoId) {
  const response = await fetch(`https://localhost:5001/ToDoCards/DelateToDoCard/${toDoId}`, {
    method: 'DELETE'
  })
  output.innerHTML = '';
  if (response) {
